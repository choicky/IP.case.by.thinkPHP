...
class CaseTypeController extends Controller {
	...
	public function listPage(){
		...
		$case_type_list = D('CaseTypeView')->relation(true)->field(true)->listPage($p,$limit);
		$this->assign('case_type_list',$case_type_list['data']);
		$this->assign('case_type_page',$case_type_list['page']);
        
        $case_group_list =   D('CaseGroup')->select();
	...
	public function add(){
		...
		$result = M('CaseTypeView')->add($data);
	...
	public function update(){
		if(IS_POST){
			...
			$case_type_list = D('CaseTypeView')->field(true)->relation(true)->getByCaseTypeViewId($case_type_id);
	...
	public function delete(){
		...
		if(IS_POST){
			...
			if(1==$yes_btn){
                ...
                $case_type_list =   M('Case')->field('case_type_id')->where($map)->find();
